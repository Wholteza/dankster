...
exports.hitme = (args, reddit, callback) => {
	...
	for (let i = 0; i < multiplyer; i++) {
		var randomSubreddit = reddit.subreddits[Math.floor(Math.random() * reddit.subreddits.length)];
...
exports.subreddits = (args, user, reddit, whitelist, callback) => {
	if (args !== null) {
		if (whitelist.users.includes(user)) {
			...
			if (argcmd === "add") {
				if (args.length === 0) {
					callback("You tried to add subreddits but didn't define any after the \"add\".");
					return;
				}
				...
				FileWrapper.writeObjectToJsonFile("reddit.json", reddit, () => {
			...
			if (argcmd === "remove") {
				if (args.length === 0) {
					callback("You tried to remove subreddits but didn't define any after the \"remove\".");
					return;
				}
				let subreddits = [];
				...
				callback(`These subreddits were removed: \r\n${OutputFormatter.wrapInBlock(OutputFormatter.toDiscordList(subreddits))}`);
				return;
			}
			if (argcmd === "list") {
				callback(`These subreddits are in the list: \r\n${OutputFormatter.wrapInBlock(OutputFormatter.toDiscordList(reddit.subreddits))}`);
				return;
			}
		} else {
		...
		if (args !== null) {
			if (args[0] === "add") {
				...
				if (args.length !== 0) {
					...
					FileWrapper.writeObjectToJsonFile("whitelist.json", whitelist, () => {
			...
			if (args[0] === "remove") {
				...
				for (let i = 0; i < whitelist.users.length; i++) {
					for (let j = 0; j < args.length; j++) {
						if (whitelist.users[i] === args[j] && i !== 0) {
							usersFormatted = usersFormatted.concat(`    ${whitelist.users[i]}\r\n`);
				...
				callback(`These users were removed from the whitelist: \`\`\`${usersFormatted}\`\`\``);
				return;
			}
			if (args[0] === "list") {
				let usersFormatted = "";
...
exports.help = (callback) => {
	var help = {
		title: "Hi, i'm a bot created by Wholteza.\r\n My possible commands are:",
		commands: [
			{
				command: "**!help**",
			...
			{
				command: "**!hitme**",
				description: "Fetch a random image from a random subreddit listed with the command !subreddits"
			},
			{
				command: "**!subreddits**",
	...
	function composeHelpMessage () {
		...
		for (let i = 0; i < help.commands.length; i++) {
			commands = commands.concat(indent + help.commands[i].command + separator + help.commands[i].description + lineEnding);
		}
